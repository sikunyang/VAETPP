...
class Batch(DotDict):
    ...
    """
    def __init__(self, inter_times: torch.Tensor, arr_times: torch.Tensor, mask: torch.Tensor, marks: torch.Tensor, time_interval_idx: torch.Tensor, **kwargs):
        self.inter_times = inter_times
        self.arr_times = arr_times
        self.mask = mask
        self.marks = marks
        self.time_interval_idx = time_interval_idx
    ...
    def from_list(sequences: List[Sequence]):
        ...
        arr_times = pad_sequence([torch.tensor(seq.arr_times) for seq in sequences], max_len=max_seq_len)
        time_interval_idx = pad_sequence([torch.tensor(seq.time_interval_idx).long() for seq in sequences], max_len=max_seq_len)
        ...
        return Batch(inter_times, arr_times, mask, marks, time_interval_idx)
