    ...
    for time_seq in timeseries:
        ...
        if not log_mode:
            for idx in range(len(time_seq)-seq_len+1):
                ...
                times = [t for (t, e, tau) in seq]
                times = [0] + np.diff(times).tolist()
                taus = [tau for (t, e, tau) in seq]
                events = [e for (t, e, tau) in seq]
                TIME_SEQS.append(taus)
...
def plt_lmbda(timeseries, model, seq_len, log_mode=False, dt=0.01, lmbda0=0.2, alpha=0.8, beta=1.0, epoch = 0):
    ...
    plt.plot([t for t, e, tau in timeseries][seq_len-1:], np.array(pred_dict[0].detach()), color='olive')
    plt.scatter([t for t, e, tau in timeseries], [-1 for _ in timeseries], color='blue')
    plt.title(epoch)
    plt.show()