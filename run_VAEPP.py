...
for seed in range(5):
    ...
    # dataset_name = 'dataset_SOF_beta2_dim12'
    # dimension = 12  # number of dimensions
    # num_prd = 5  # number of time intervals
    # batch_size = 5
    ...
    max_epochs = 700#00    # For how many epochs to train
    ...
    def aggregate_loss_per_event_over_dataloader(dl, mode):
        ...
        with torch.no_grad():
            for batch in dl:
                ...
                # total_loss += -model.log_prob_with_dynamic_graph_v1(batch, mode).sum()
                dataloglik,se,loss_mark, pred_num = model.log_prob_with_dynamic_graph_v1(batch, mode, 1, dataset_name)
    ...
    for epoch in range(max_epochs):
        ...
        for batch in dl_train:
            ...
            dataloglik, se, loss_mark,pred_num = model.log_prob_with_dynamic_graph_v1(batch, mode, epoch, dataset_name)
            # loss = -dataloglik
            # loss = loss.mean()
            ##
        ...
        with torch.no_grad():
            loss_val, negllk_val, acc_pred_mark = aggregate_loss_per_event_over_dataloader(dl_val, mode)
            training_val_losses.append(loss_val)
    ...
    with torch.no_grad():
        final_loss_train, final_se_train, final_acc_train = aggregate_loss_per_event_over_dataloader(dl_train, mode)
        final_loss_val, final_se_val, final_acc_val = aggregate_loss_per_event_over_dataloader(dl_val, mode)
        final_loss_test, final_se_test, final_acc_test = aggregate_loss_per_event_over_dataloader(dl_test, mode)
    ...
    #
    # file1 = open(filepath + '/results/TestNLL-VAETPP-' + dataset_name +'-'+ str(seed) + '.txt', "w")
    ...
    # file1.close()
    #
    # file1 = open(filepath + '/results/TestACC-VAETPP-' + dataset_name +'-'+ str(seed) + '.txt', "w")
